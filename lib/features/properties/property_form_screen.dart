...
class _PropertyFormScreenState extends ConsumerState<PropertyFormScreen> {
  ...
  void initState() {
    ...
    if (widget.propertyToEdit != null) {
      ...
      _selectedType = [AppConstants.typeKiwanja, AppConstants.typeNyumba, AppConstants.typeGari].contains(p.type) ? p.type : AppConstants.typeKiwanja;
      ...
      _selectedStatus = [AppConstants.propertyAvailable, AppConstants.propertyReserved, AppConstants.propertyUnderProcess].contains(p.status) ? p.status : AppConstants.propertyAvailable;
    }
  }
  ...
  Future<void> _submitProperty() async {
    ...
    List<String> finalImages = widget.propertyToEdit?.images.isNotEmpty == true ? widget.propertyToEdit!.images : [];
    if (_selectedImageBytes != null && _selectedImage != null) {
      final fileName = '${DateTime.now().millisecondsSinceEpoch}_${_selectedImage!.name}';
      final url = await repo.uploadPropertyImage(_selectedImageBytes!, fileName);
      if (url != null) {
        finalImages = [url];
      }
    } else if (finalImages.isEmpty) {
      finalImages.add('https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&q=80');
    }

    final double price = double.tryParse(_priceCtrl.text.replaceAll(',', '')) ?? 0.0;

    final newProperty = PropertyModel(
      id: widget.propertyToEdit?.id ?? 'prop_${DateTime.now().millisecondsSinceEpoch}',
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(widget.propertyToEdit != null ? 'Property updated successfully!' : 'Property created successfully!')),
      );
      context.pop();
    }
  }
            ...
            Step(
              ...
              content: Column(
                children: [
                  ...
                  const Text('Property Image:', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
            ...
            Step(
              ...
              content: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  DropdownButtonFormField<String>(
                    value: TanzaniaLocations.allRegions.contains(_regionCtrl.text) ? _regionCtrl.text : 'Dar es Salaam Region',
                    isExpanded: true,
                  ...
                  Builder(
                    builder: (context) {
                      final dists = TanzaniaLocations.getDistricts(_regionCtrl.text.isEmpty ? 'Dar es Salaam Region' : _regionCtrl.text);
                      final validDist = dists.contains(_districtCtrl.text) ? _districtCtrl.text : (dists.isNotEmpty ? dists.first : null);
                  ...
                  AppTextField(label: 'Specific Location / Street Address', hint: 'e.g. Mbuyuni Street, Near Beach', controller: _locationCtrl),
                  const SizedBox(height: 12),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 16),
                    Text('Uploading Image & Saving...', style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
